...
def main(args):
    ...
    source_classes = [[], [], []]
    source_classes[0] = range(48)
    source_classes[1] = range(48)
    source_classes[2] = range(48)
    ...
    for j, the_source in enumerate(args.source):
        train_source_dataset = dataset(root=source_root_list[j], task=the_source, filter_class=source_classes[j],
    ...
    val_loader = []
    for j, the_source in enumerate(args.source):
        val_source_dataset = dataset(root=source_root_list[j], task=the_source, filter_class=source_classes[j],
    ...
    for j, the_target in enumerate(args.target):
        test_dataset = dataset(root=target_root, task=the_target, filter_class=ST1,
